       ...
       var body = '';
        ...
        response.on('end', function() {
          ...
          https.get(dealURL, function(response) {
        		...
        		response.on('end', function() {
        			var dealBody = JSON.parse(body).deals;

        			for (var i = 0; i < dealBody.length; i++) {
        				if (dealBody[i].properties) {
        					var properties = dealBody[i].properties;
									if (properties.first_name) {
										console.log(properties.first_name.value);
									}
        				}
